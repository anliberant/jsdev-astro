...
export class HtmlTagRemoverController extends BaseConverterController<HtmlTagRemoverElements> {
  ...
  private loadSample(): void {
    ...
    const inputLength = this.elements.htmlInput?.value?.length || 0;
    const outputLength = this.elements.markdownOutput?.value?.length || 0;
    ...
    try {
      ...
      if (input.trim()) {
        if (this.currentMode === 'html-to-markdown') {
          result = this.htmlToMarkdown(input);
        } else {
          result = this.markdownToHtml(input);
        }
      }
      ...
      this.elements.markdownOutput.value = `Error: Could not convert content`;
    }
  }
  ...
  private async copy(): void {
    ...
    try {
      await navigator.clipboard.writeText(this.elements.markdownOutput.value);
      this.showCopySuccess();
    } catch (error) {
      console.error('Copy failed:', error);
    }
